...
### 4.5 `needs_analyze_adv` (LLM, одна работа) — второй разбор другим вопросом
...
- **выход:** список функций-кандидатов, у каждой входная фраза из поиска, доказательство того, что
  за это платят, одно отличие нашей версии, канал первых пользователей, объём работ и **чем
  опровергнуть**. Вердикт — по лучшей функции, не по работе.
- **бесплатен по разобранной работе:** выдача берётся из общего оплаченного кэша.

Два разбора живут рядом и не заменяют друг друга; расхождение между ними — сигнал, а не ошибка.

### 4.6 `needs_season` (XMLRiver + LLM, одна работа)
...
### 4.7 `needs_adjacent` (XMLRiver + LLM, одна работа)
...
### 4.8 `needs_dump` (XMLRiver, без LLM) — полная выгрузка топ-10
...
- **грузим углы, а не все фразы.** Топ-10 по шести близким формулировкам совпадает на 70–80% —
  ...
  фраза кандидата из `analyze_adv`, «как это делают руками», «бесплатно / без регистрации»,
...
### 4.9 `stopwords_scan` (LLM, одна ветка)
...
- `POST /api/needs/analyze_adv` `{tree_id, work}` — второй разбор: одна функция, вход, деньги.
...
2а. **Стоп-слова** — что не покупаем (§4.7).
3. **Лог** — окно логов (§9).
...
- На строке работы: оценка, частота, число формулировок, метки `ЩЕЛЬ` / `НЕ ЯСНО` / `занято: …` /
  `?выдача`, вердикт после разбора и **одно меню «Действие»**: три запуска (`Analyze`, сезонность,
  смежные ключи) и **все накопленные отчёты ссылками**. Кнопок много, а места на строке нет.
