...
### У списка есть владелец

Одно и то же слово запрещено не везде. «Учебник» в ветке `pdf` — это «скачать конкретную книгу»,
работы там нет; в школьной ветке то же слово — сама суть спроса. Поэтому запрет хранится вместе с
**областью**, а не сам по себе.

| Владелец | Где действует |
|---|---|
| общий список | на любой фразе дерева |
| **узел** | на самом узле и на всём, что от него уточняется |
| **домен** | на ветках **всех** ключей домена |

Принадлежность области считается **по словам**, а не обходом рёбер: уточнение у Вордстата всегда
содержит слова родителя (на этом же построен разбор пула на детей). Проверка от этого дешёвая и не
зависит от того, какими рёбрами фраза попала в дерево.

**Инварианты** (`wscore.stopword_violations` их же и проверяет):

- **И1** — у записи ровно один владелец: общий, узел или домен; адресат непуст тогда и только
  тогда, когда владелец не общий.
- **И2** — у узла, **принятого в домен**, своего списка не бывает. Иначе один запрет жил бы в двух
  местах и расходился: домен — это и есть общая точка зрения на свои ключи.
- **И3** — создание домена и приём узла в домен **переносят** список узла домену; совпавшие слова
  схлопываются (побеждает запись домена), у узла не остаётся ничего.
- **И4** — слово действует только в своей области; в чужой ветке то же слово ничего не запрещает.
- **И5** — адресат существует: узел — в дереве, домен — среди доменов.
...
- одиночный `load` по такой фразе отбивается 422: это ровно та покупка, от которой список и
  отказывается, а краул её и так пропускает;
...
**Сборка потребностей по набору веток:**

- единица сборки — **набор веток**, а не одна: одну работу пишут по-разному («pdf в ворд» и
  «пдф в ворд»), и порознь ветки дают два дерева-двойника с разделённым пулом — ровно то, от
  чего уходили, когда единицей стала ветка целиком;
- кнопка **над доменом** собирает одно дерево по всем его ключам; ветки берёт сервер из состава
  домена, клиент их не пересобирает;
- кнопка на узле осталась прежней — это набор из одной ветки;
- **готовность считается по всем веткам набора**: недогруженная ветка исказила бы сравнение,
  поэтому отбивается весь набор с перечислением виновных. Заняты на время сборки тоже все;
- главной считается самая частотная ветка: ею подписано дерево и от неё берётся его id.

**Домен на узле:**

- узел вне домена показывает зелёную кнопку **«Домен»**: она раскрывает панель, где его можно
  принять в существующий домен или сделать домен из него самого (без названия — по фразе);
- узел в домене вместо кнопки показывает **имя своего домена**: второго домена у него не будет,
  и предлагать его незачем;
- панель прямо говорит, что стоп-слова узла переедут домену — переезд не должен быть сюрпризом;
- новый состав доменов приезжает всем открытым клиентам событием `roots`, поэтому член домена
  не остаётся нарисованным отдельным корнем.

**Стоп-слова:**

- **выбор владельца списка** первой строкой: общий список, домены, корни вне доменов. Домен
  выделен зелёным — это же зелёное «Домен» стоит на узле, поэтому владельца видно, а не читают;
- показывается и правится список **только выбранного владельца**: слово чужой области на эту
  ветку не действует, и в её списке ему не место;
- поле узла и кнопка **«Анализ стоп-слов»** — задача уходит в LLM, кнопка блокируется на время
  разбора; выбор узла-владельца подставляет его в поле разбора;
...
- под принятым списком — **поле для своего слова**: список наполняется руками, не дожидаясь
  разбора. Ввод режется на слова (разделители — пробел, запятая, точка с запятой), потому что
  сравнение идёт со **словом** фразы: пара слов не совпадёт никогда, и сервер такую запись
  отбивает 422, а не принимает молча мёртвой;
