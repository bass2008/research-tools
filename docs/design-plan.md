...
- **воронка считается от итога пула** (двери берутся из таблицы «Функций» и перепроверяются по
  фразам работы), и перед спецификацией идёт **обратный счёт**: какой вход нужен, чтобы окупить
  бюджет при выбранной цене. Пул на порядок меньше требуемого — сразу `SKIP`, спецификацию писать
  незачем.
...
Виды артефактов работ: `analyze`, `analyze_adv`, `analyze_product`, `season`, `adjacent`, `dump`.
...
- `POST /api/needs/favorite` `{tree_id, work, favorite}` — ручной лайк работы.
