...
def test_arcanum_page_points_to_the_year_article_without_retelling_it():
    """Год — отдельная тема: страница аркана только ссылается на годовую статью.

    Раньше тут была вкладка «На год» с пересказом первого абзаца годовой статьи: совпадение
    47–80% шестисловных шинглов, и по запросу «7 аркан на год» Яндекс показывал страницу аркана
    ...
    assert "На год" not in [t.strip() for t in tabs], f"вкладка «На год» на месте: {tabs}"
