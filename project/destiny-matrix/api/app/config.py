...
class Settings(BaseSettings):
    ...
    # Общий режим доступа, независимый от языка: при включении разбор открыт всем.
    # По умолчанию выключен. Фронт получает согласованное значение через свою переменную
    # сборки NEXT_PUBLIC_ALL_FREE_WITHOUT_PAYMENT; отдельного бесплатного EN-контура нет.
    all_free_without_payment: bool = False

    # Резервный язык при отсутствии поддерживаемого Accept-Language. Язык запроса хранится
    # в ContextVar; фоновые операции восстанавливают язык из данных операции.
    site_lang: str = "ru"
    ...
    # Общий с BFF список доменов, языков и разрешённых платёжных подключений.
    site_profiles: str = "[]"
    ...
    # Ключи включают подключение Т-Банка только в явно разрешающих его профилях.
