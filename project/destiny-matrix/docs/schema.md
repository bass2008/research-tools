...
Для доменов одного окружения база общая. Языковая модель описана в
[runtime-localization.md](runtime-localization.md); ниже — её поля хранения:

- `matrices.title = NULL` означает автоматическую подпись на языке запроса; ручные и прежние
  сохранённые названия остаются строками.
- `payments.tariff_body` содержит снимок тарифа и `locale` операции для последующих писем.
  Отдельного пользователя или права доступа для каждого языка нет.
- `report_jobs.locale` — язык PDF, учитываемый при поиске готового отчёта и дедупликации.
  При добавлении колонки прежним заданиям назначается язык прежней базы.

Полный актуальный состав таблиц задаёт `api/app/models.py`; SQL ниже — обзор основных
сущностей, а не полный миграционный скрипт.
...
Сессия админа, вошедшего под пользователем, присутствие не отмечает: её токен помечен `gho`, и
`pulse` такую отметку не копит. `last_seen_at` — появление самого человека, а не того, кто смотрел
его кабинет.
