...
`matrix` | хранить и открывать матрицы в кабинете; без него держим одну |
...
**Как разовое право находит свою дату.** Дата рождения в платёжный запрос не входит — так обещано
в оферте, — поэтому право приходит с `matrix_id IS NULL`. Оно привязывается один раз: на оплате — к
последней сохранённой матрице, если она уже есть; иначе — к первой матрице, которую сохранят после
оплаты (`access.bind_single`). Привязка происходит только на записи; чтение разбора прав не меняет.
