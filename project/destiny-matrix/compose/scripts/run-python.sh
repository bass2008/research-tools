#!/usr/bin/env bash
# Запуск без докера: uvicorn напрямую и собранный фронт на node. Для отладки в IDE и правок
# питона на живую — образ пересобирать не надо.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# порты могли остаться заняты прошлым запуском
...
# NODE_ENV=development намеренно: в production кука сессии уходит с флагом secure и по http
# вход не работает вовсе
