...
# Язык развёртки — первым аргументом: `run-python.sh ru`, `run-python.sh en`.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
export NEXT_PUBLIC_SITE_LANG="$LANG_CODE"
# На английском контуре кассы пока нет: разбор открыт всем (docs/foreign-acquiring.md).
...
echo "сайт: http://127.0.0.1:$WEB_PORT  ·  язык: $SITE_LANG  ·  логи: $LOGS"
