...
import { forLocale as localizedCharacter } from "./character";
import { forLocale as localizedCharacterTypes, type CharacterPositionKey } from "./characterTypes";
...
import { forLocale as localizedMethodLabels } from "./i18n/methodLabels";
...
import {
  forLocale as localizedSectionReadings,
  type PersonalSectionKey
} from "./sectionReadings";
import { forLocale as localizedSectionReadingShared } from "./sectionReadingShared";
import {
  forLocale as localizedSectionResolver,
...
export const forLocale = localized((L: Locale) => {
  ...
  const { buildCharacterReading, characterRoleTemplate } = localizedCharacter(L);
  const { characterHref } = localizedCharacterTypes(L);
  const { arcanumInPosition, karmicTail } = localizedContent(L);
  const { buildSectionReading, sectionReadingHref, sectionRoleTemplate } = localizedSectionReadings(L);
  const { PERSONAL_SECTION_KEYS } = localizedSectionReadingShared(L);
  const { CATALOG, FREE_POSITION_KEYS, arcanumHref } = localizedPublicSpec(L);
  const { positionArcanumHref, positionArcanumLabel, registryItem } = localizedPositionArcanum(L);
  const { positionKeys, resolveSectionPositions } = localizedSectionResolver(L);

  // Схема (селекторы и ключи точек) — контракт, слова — словарь выбранного языка. Подпись позиции
  ...
  const SPEC: SectionSpec[] = CATALOG.map((meta) => {
  ...
  const FREE_KEYS: string[] = SPEC.filter((section) => section.access === "free").map((section) => section.key);

  const PAID_KEYS: string[] = SPEC.filter((section) => section.access === "paid").map((section) => section.key);

  const SECTION_KEYS: string[] = SPEC.map((section) => section.key);

  /** Собрать разделы. При unlocked=false платные приходят без позиций — только анонс. */
  function build(matrix: Matrix, unlocked = false): SectionOut[] {
   ...
   *  там быть не должно. */
  function withPositionArticles(matrix: Matrix, sections: SectionOut[]): SectionOut[] {
  ...
  function freePositionArticles(): PositionArticles {
  ...
  function freePositionTexts(): PositionTexts {
  ...
  function referencedArcana(matrix: Matrix): number[] {
  ...
  function sectionByKey(key: string): SectionSpec | undefined {
    return SPEC.find((section) => section.key === key);
  }
  return { arcanumHref, SPEC, FREE_KEYS, PAID_KEYS, SECTION_KEYS, build, withPositionArticles, freePositionArticles, freePositionTexts, referencedArcana, sectionByKey };
});

// Compatibility for callers that explicitly use the deployment default.
export const { SPEC, FREE_KEYS, PAID_KEYS, SECTION_KEYS, build, withPositionArticles, freePositionArticles, freePositionTexts, referencedArcana, sectionByKey } = forLocale(defaultLocale);
