...
import { forLocale as localizedMatrixMap } from "./matrixMap";
...
export const forLocale = localized((L: Locale) => {
  const { arcanumContent, indexedKarmicTailKeys, indexedPositionArcanumRows, karmicTail, positionArcanumRows, positionContent } = localizedContent(L);
  const { karmicTailHref, positionHref } = localizedEncyclopedia(L);
  const { sectionLineKeys } = localizedMatrixMap(L);
  ...
  function indexedRegistryItems(): RegistryItem[] {
    return indexedPositionArcanumRows();
  }

  function registryItem(position: string, arcanum: number): RegistryItem | null {
  ...
  function positionArcanumSiblings(position: string, arcanum: number): RegistryItem[] {
   ...
   *  показывать нельзя. */
  function positionArcanumLabel(item: RegistryItem): string {
  ...
  function positionArcanumHref(position: string, arcanum: number): string {
    return `${positionHref(position)}/${arcanum}`;
  }

  function buildPositionArcanum(position: string, arcanum: number): PositionArcanumReading {
  ...
  return { registryItems, indexedRegistryItems, registryItem, positionArcanumSiblings, positionArcanumLabel, positionArcanumHref, buildPositionArcanum };
});

// Compatibility for callers that explicitly use the deployment default.
export const { registryItems, indexedRegistryItems, registryItem, positionArcanumSiblings, positionArcanumLabel, positionArcanumHref, buildPositionArcanum } = forLocale(defaultLocale);
