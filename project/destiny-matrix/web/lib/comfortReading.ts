...
export const forLocale = localized((L: Locale) => {
  ...
  const COMFORT_ROLE_META: Record<
    ComfortPositionKey,
  ...
  function buildComfortConclusion(items: ReadingRole[]): ReadingConclusion {
  ...
  return { COMFORT_ROLE_META, comfortRoleMeta, comfortHref, repeatedSummary, buildComfortConclusion };
});

// Compatibility for callers that explicitly use the deployment default.
export const { COMFORT_ROLE_META, comfortRoleMeta, comfortHref, repeatedSummary, buildComfortConclusion } = forLocale(defaultLocale);
