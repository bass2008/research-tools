...
export const forLocale = localized((L: Locale) => {
  ...
  function emailProblem(mail: string): EmailProblem | null {
  ...
  function emailProblemMessage(problem: EmailProblem): string {
    return MESSAGES[problem];
  }
  return { normalizeEmail, emailProblem, isValidEmail, emailError, emailProblemMessage };
});

// Compatibility for callers that explicitly use the deployment default.
export const { normalizeEmail, emailProblem, isValidEmail, emailError, emailProblemMessage } = forLocale(defaultLocale);
