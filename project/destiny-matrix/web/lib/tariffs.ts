...
// (энциклопедия, 5 544 матрицы) собрана заранее: до ответа BFF её кнопки пишут «уточняется»,
// живую цену подставляет `TariffsProvider`. Зашитого прайса в разметке нет нигде.
 ...
 *
 * Запасную цену отсюда не отдаём. Она подставлялась ровно тогда, когда API недоступен, то есть
 * когда платёж всё равно не пройдёт: страница называла 250 ₽, по которым нельзя купить.
 * Страница обязана открыться и без прайса — но без числа, а не с числом из кода.
 */
