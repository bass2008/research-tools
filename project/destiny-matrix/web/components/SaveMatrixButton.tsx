 ...
 * сервер: в браузер их толкования не приходят вовсе. Отказ по лимиту тарифа показывается
 * отдельным крючком `limit-message`: раньше 402 оседал в безымянном блоке ошибки, и человек
 * видел «что-то не так» вместо «лимит тарифа исчерпан».
 */
  ...
  const save = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 402) {
        ...
        setLimit(
          `${err.message} По вашему тарифу лимит матриц исчерпан — откройте разбор по уже ` +
            "сохранённой матрице или смените тариф.",
        );
        return;
      }
      setError(
